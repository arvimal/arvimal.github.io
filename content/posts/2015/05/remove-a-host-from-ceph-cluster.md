...
```bash # ceph osd crush remove osd.1 ```

e) Remove the OSD authentication key

```bash # ceph auth del osd.1 ```
...
```bash # ceph-deploy purge hp-m300-9
...
```bash # ceph osd getcrushmap -o /tmp/crushmap ```

h) De-compile the CRUSH map

```bash # crushtool -d /tmp/crushmap -o crush\_map ```
...
```bash # crushtool -c crush\_map -o /tmp/crushmap # ceph osd setcrushmap -i /tmp/crushmap ```

k) A 'ceph osd tree' looks much cleaner now :)

```bash # ceph osd tree

\# id         weight             type         name                up/down        reweight -1             0.07999            root         default -2            0.01999                        host hp-m300-5 0            0.009995                    osd.0                down        0 4            0.009995                    osd.4                 down         0 -4            0.06                        host hp-m300-4 2            0.04999                        osd.2                 up             1 3            0.009995                    osd.3                 up             1 ```
