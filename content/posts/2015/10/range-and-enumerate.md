...
```python colors = ["yellow", "red", "blue", "white", "black"]

for i in range(len(colors)): print(i, colors[i]) ```

This should output:

```bash (0, 'yellow') (1, 'red') (2, 'blue') (3, 'white') (4, 'black') ```
...
```bash 1: yellow 2: red 3: blue 4: white 5: black ```
...
```python colors = ["yellow", "red", "blue", "white", "black"] print(list(enumerate(colors))) ```
...
```python for i, color in enumerate(colors): print('%d: %s' % (i, color)) ```
...
```python for i, color in enumerate(colors, start=1): print('%d: %s' % (i, color)) ```
